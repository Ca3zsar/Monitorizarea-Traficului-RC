...
void create_newsletter()
{
  sprintf(newsletter[0].weather,"Vreme racoroasa. Sanse mici de ploaie");
  sprintf(newsletter[0].sport,"Manchester City 0 - 1 Chelsea");
  newsletter[0].prices[0]=4.65;
  newsletter[0].prices[1]=4.82;
  newsletter[0].prices[2]=4.43;

  sprintf(newsletter[1].weather,"Vreme calduroasa. Sanse minime de ploaie");
  sprintf(newsletter[1].sport,"Bayern Munchen 2 - 3 Real Madrid");
  newsletter[1].prices[0]=4.93;
  newsletter[1].prices[1]=5.02;
  newsletter[1].prices[2]=4.56;
}
...
int announce_all(struct args *arg, char *alert) {
  ...
  for (int fd = 4; fd <= fdNumber; fd++) {
    if (!closed[fd] && FD_ISSET(fd, &writeFD) && fd != arg->clientId) {
      ...
      message =
          (char *)malloc(strlen(alert) + strlen("[]  : ") + strlen(timestr) +
                         strlen(clients[arg->clientId].username) + 1);
      sprintf(message, "[%s] %s : %s", timestr, clients[arg->clientId].username,
              alert);
...
void news_operations(struct args *arg) {
  int type=3;

  while (waitFor);

  char news_string[200];
  sprintf(news_string,"Vreme : %s \nSport : %s \nPreturi combustibil: %0.2f , %0.2f, %0.2f",
        newsletter[newsIndex].weather,newsletter[newsIndex].sport,newsletter[newsIndex].prices[0],
        newsletter[newsIndex].prices[1],newsletter[newsIndex].prices[2]);

  newsIndex = 1 - newsIndex;

  write(arg->clientId, &type, sizeof(int));
  if (!write_to_client(arg->threadId, arg->clientId, news_string)) {
    arg->active = 0;
