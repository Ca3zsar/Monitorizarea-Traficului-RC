...
int write_to_server(int socketD, char *message) {
  ...
  int written;
  if ((written = write(socketD, &msgLength, sizeof(int))) <= 0) {
...
void display_alert(main_window::obj_to_pass *currentObject, char *alert) {
  ...
  QMetaObject::invokeMethod(currentObject->current->ui->alert_log, "setText",
                            Qt::AutoConnection, Q_ARG(QString, existent));
}

void display_limit(main_window::obj_to_pass *currentObject, char *speed_msg) {
  QString limitString;
  limitString.append(speed_msg);
  QMetaObject::invokeMethod(currentObject->current->ui->speed_label, "setText",
                            Qt::AutoConnection, Q_ARG(QString, limitString));
}

void display_speed(main_window::obj_to_pass *currentObject, float speed) {
  QString speedString;
  char speed_char[10];
  sprintf(speed_char, "%0.2f", speed);

  speedString.append("Your speed is ");
  speedString.append(speed_char);
  speedString.append(" km\\h");
  QMetaObject::invokeMethod(currentObject->current->ui->cur_speed_label,
                            "setText", Qt::AutoConnection,
                            Q_ARG(QString, speedString));
}

void display_news(main_window::obj_to_pass *currentObject, char *news) {
  QString newsString;
  newsString.append(news);

  QMetaObject::invokeMethod(currentObject->current->ui->news_text, "setText",
                            Qt::AutoConnection, Q_ARG(QString, newsString));
}

void display_disconnected(main_window::obj_to_pass *currentObject) {
  QMetaObject::invokeMethod(currentObject->current->ui->disconnected_label,
                            "setVisible", Qt::AutoConnection, Q_ARG(bool, 1));
  keepRunning = 0;
}
...
void *write_speed(void *currentObject) {
  ...
  int socketD =
      ((main_window::obj_to_pass *)currentObject)->information.socketD;
  ...
  while (keepRunning) {
    ...
    if (!write(socketD, &type, sizeof(int))) {
      ...
      display_disconnected((main_window::obj_to_pass *)currentObject);
      break;
    }
    ...
    display_speed((main_window::obj_to_pass *)currentObject,
                  speeds[index % 10]);
...
void *read_news(void *currentObject) {
  ...
  while (keepRunning) {
    ...
    if (type == 1) {
      ...
      display_limit((main_window::obj_to_pass *)currentObject, answer);
    }
    if (type == 2) {
    ...
    if (type == 3) {
      ...
      display_news((main_window::obj_to_pass *)currentObject, news);
    }
  }
...
void *ask_for_news(void *currentObject) {
  ...
  while (keepRunning) {
    ...
    if ((written = write(socketD, &type, sizeof(int))) <= 0) {
      display_disconnected((main_window::obj_to_pass *)currentObject);
      break;
    }
  }
...
void *lobby(void *object) {
  currentObject =
      (main_window::obj_to_pass *)malloc(sizeof(main_window::obj_to_pass));
  ...
  if (currentObject->information.subscribed) {
    ...
    pthread_create(&newsThread, NULL, &ask_for_news, (void *)currentObject);
  }
  keepRunning = 1;
...
void main_window::showEvent(QShowEvent *ev) {
  ...
  this->ui->disconnected_label->setVisible(0);

  this->thisObj->current = (main_window *)malloc(sizeof(main_window));
  this->thisObj->current = this;

  if (!this->thisObj->information.subscribed) {
    this->ui->news_label->setEnabled(0);
    this->ui->news_label->setVisible(0);
    this->ui->news_text->setEnabled(0);
    this->ui->news_text->setVisible(0);
  }
...
void main_window::on_report_btn_clicked() {
